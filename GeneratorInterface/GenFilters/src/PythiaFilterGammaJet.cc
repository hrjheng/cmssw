...
PythiaFilterGammaJet::PythiaFilterGammaJet(const edm::ParameterSet &iConfig) : token_(consumes<edm::HepMCProduct>(edm::InputTag(iConfig.getUntrackedParameter("moduleLabel", std::string("generator")), "unsmeared"))),
...
bool PythiaFilterGammaJet::filter(edm::Event &iEvent, const edm::EventSetup &iSetup)
{
    ...
    if (myGenEvent->signal_process_id() == 14 || myGenEvent->signal_process_id() == 29)
    {
        ...
        for (HepMC::GenEvent::particle_const_iterator p = myGenEvent->particles_begin(); p != myGenEvent->particles_end(); ++p)
        {
            ...
            if ((*p)->pdg_id() == 22 && (*p)->status() == 1 && (*p)->momentum().perp() > ptSeed && std::abs((*p)->momentum().eta()) < etaMax)
                seeds.push_back(*p);
        }
            ...
            for (HepMC::GenEvent::particle_const_iterator p = myGenEvent->particles_begin(); p != myGenEvent->particles_end(); ++p)
            {
                ...
                if (inEB)
                {
                    if (std::abs(eta - etaPhoton) > deltaEB ||
                        std::abs(deltaPhi(phi, phiPhoton)) > deltaEB)
                        continue;
                }
