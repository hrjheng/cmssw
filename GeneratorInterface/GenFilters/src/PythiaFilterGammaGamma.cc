...
PythiaFilterGammaGamma::PythiaFilterGammaGamma(const edm::ParameterSet &iConfig) : token_(consumes<edm::HepMCProduct>(edm::InputTag(iConfig.getUntrackedParameter("moduleLabel", std::string("generator")), "unsmeared"))),
                                                                                   ...
                                                                                   promptPtThreshold(iConfig.getUntrackedParameter<double>("PromptPtThreshold"))
{
    ...
    if (maxEvents != 0)
        edm::LogInfo("PythiaFilterGammaGamma::PythiaFilterGammaGamma") << "WARNING, ignoring unsuported option, maxEvents = " << maxEvents << endl;
}
                ...
                if (acceptPrompts)
                {
                    ...
                    if ((*itSeed)->momentum().perp() > promptPtThreshold)
                    {
                        ...
                        while (mom->pdg_id() == this_id)
                        {
                            ...
                            const GenParticle *mother = mom->production_vertex() ? *(mom->production_vertex()->particles_in_const_begin()) : nullptr;
